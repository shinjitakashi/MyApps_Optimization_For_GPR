    ...
    for i in [0,1]:
        ...
        if (i):
            for e in range(len(eventtrigger)):
                ...
                for i in range(N):
                    np.savetxt(save_dir+'params_for_agent'+str(i)+'_at_E='+str(eventtrigger[e])+'.dat', multi_gp[i].kernel.param)

                np.savetxt(save_dir+'objective_function_E='+str(eventtrigger[e])+'.dat', objective_function_array[e])
